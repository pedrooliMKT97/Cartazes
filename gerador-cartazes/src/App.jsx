...
import { 
  ...
  Eye, CheckCircle, RefreshCcw, X, List, Sliders, MoveVertical, Save, Bookmark, Loader, File
} from 'lucide-react';
  ...
  const send = async () => {
      ...
      try {
          ...
          for(let i=0; i<bulkProducts.length; i++) {
              ...
              if(el) { const canvas = await html2canvas(el, {scale:2, useCORS:true}); if(i>0) pdf.addPage(); pdf.addImage(canvas.toDataURL('image/png'), 'PNG', 0, 0, w, h); }
        ...
        <div className="flex-1 flex overflow-hidden">
            ...
            <div className="w-1/2 h-full bg-slate-100 p-6 overflow-y-auto">
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="bg-white p-4 rounded shadow"><h3 className="font-bold text-slate-700 mb-2">Recentes</h3><div className="space-y-1 max-h-40 overflow-y-auto">{files.map(f=><div key={f.id} className="flex justify-between text-xs p-1 border-b"><span>{f.title}</span><button onClick={()=>handleDelete(f.id)} className="text-red-500"><Trash2 size={12}/></button></div>)}</div></div>
                </div>
            </div>
        </div>
...
// ============================================================================
// 6. LOJA LAYOUT (CORRIGIDO PARA NÃO TRAVAR)
// ============================================================================
